...
static fixed_point_t simulate(const uint32_t table, char player)
{
    ...
    while (1) {
        ...
        kfree(moves);
        temp_table = SET_VAL_INDEX(temp_table, move, current_player);
        char win;
        if ((win = check_win(temp_table)) != 0)
...
static int expand(struct node *node, const uint32_t table)
{
    ...
    for (int i = 0; i < n_moves; i++) {
        node->children[i] =
            new_node(moves[i], node->player ^ PLAYER1 ^ PLAYER2, node);
    }
    kfree(moves);
    return n_moves;
}
...
int mcts(const uint32_t table, char player)
{
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        while (1) {
            if ((win = check_win(temp_table)) != 0) {
                fixed_point_t score =
                    calculate_win_value(win, node->player ^ PLAYER1 ^ PLAYER2);
            ...
            if (!node)
                return -1;
            temp_table = SET_VAL_INDEX(temp_table, node->move,
                                       node->player ^ PLAYER1 ^ PLAYER2);
        }
    }
